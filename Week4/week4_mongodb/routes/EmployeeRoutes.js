...
//http://localhost:8081/employees/firstname/kippy
app.get('/employees/firstname/:name', async (req, res) => {
  const name = req.params.name

  //for case insensitive search
  //const employees = await employeeModel.find({firstname : new RegExp(name, 'i')});
  ...
  //console.log(`virtual field - employees[0].fullname: ${employees[0].fullname}`)

  //Using Instance method
 // console.log(`instance method - employees[0].getFullName(): ${employees[0].getfullname()}`)
...
//http://localhost:8081/employees/search?firstname=delano&lastname=bee
app.get('/employees/search', async (req, res) => {
  ...
  if(Object.keys(req.query).length != 2){
    ...
    //const employees = await employeeModel.find({ $and: [{firstname : new RegExp(fname, 'i')}, {lastname : new RegExp(lname, 'i')}]});
...
app.get('/employees/test', async (req, res) => {
  try {
    const employees = employeeModel
                        ...
                        .where('firstname').in(['kippy', 'delano'])
                        .limit(10)
                        .sort('-salary')
